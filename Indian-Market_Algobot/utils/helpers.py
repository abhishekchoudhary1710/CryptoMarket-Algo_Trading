...
def is_market_open(now=None):
    ...
    Uses MARKET_OPEN_HOUR/MINUTE and MARKET_CLOSE_HOUR/MINUTE from settings.
    """
    ...
    market_open_time = now.replace(hour=settings.MARKET_OPEN_HOUR, minute=settings.MARKET_OPEN_MINUTE, second=0, microsecond=0)
    market_close_time = now.replace(hour=settings.MARKET_CLOSE_HOUR, minute=settings.MARKET_CLOSE_MINUTE, second=0, microsecond=0)
    return market_open_time <= now <= market_close_time

def is_before_market_open(now=None):
    """
    Return True if current time is before market open.
    Uses MARKET_OPEN_HOUR/MINUTE from settings.
    """
    ist = pytz.timezone('Asia/Kolkata')
    if now is None:
        now = datetime.now(ist)
...
def get_today_date_range():
    ...
    # Market open time from settings
    from_time = f"{settings.MARKET_OPEN_HOUR:02d}:{settings.MARKET_OPEN_MINUTE:02d}"
...
def fetch_scripmaster_data():
    ...
    try:
        ...
        logger.info("[SUCCESS] Download complete, parsing JSON data...")
        data = response.json()
        logger.info(f"[SUCCESS] Fetched {len(data)} records from ScripMaster")
        logger.info("=" * 60)
        return data
    except requests.exceptions.Timeout:
        logger.error("[FAILED] Request timed out after 30 seconds")
        return None
