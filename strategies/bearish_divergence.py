...
class ThreeTopEntryManager:
    ...
    def _is_swing_high(self, candles, idx):
        """Check if candle at idx is a swing high (same swing-logic helper)."""
    ...
    def _is_swing_low(self, candles, idx):
        """Check if candle at idx is a swing low (same swing-logic helper)."""
        if idx <= 0 or idx >= len(candles) - 1:
            return False
...
class BearishDivergenceStrategy:
    ...
    def __init__(
        ...
        order_manager=None,
        broker=None,
        futures_1m_data=None,
        primary_symbol=None,
        secondary_symbol=None,
    ):
        """
        Initialize the strategy with primary and secondary data streams.

        Args:
            spot_1m_data: LiveOHLCVData instance for primary 1-minute candles
            spot_5m_data: LiveOHLCVData instance for primary 5-minute candles
            futures_5m_data: LiveOHLCVData instance for secondary 5-minute candles
            order_manager: Optional order manager injected by engine
            broker: Optional broker reference
            futures_1m_data: LiveOHLCVData instance for secondary 1-minute candles
            primary_symbol: Override for primary symbol label
            secondary_symbol: Override for secondary symbol label
        """
        ...
        self.broker = broker
        self.primary_symbol = primary_symbol or getattr(settings, "DIVERGENCE_PRIMARY_LABEL", "SPOT")
        self.secondary_symbol = secondary_symbol or getattr(settings, "DIVERGENCE_SECONDARY_LABEL", "FUTURES")
        self._primary_source_aliases = {"spot", "primary", str(self.primary_symbol).lower()}
        self._secondary_source_aliases = {"futures", "future", "secondary", str(self.secondary_symbol).lower()}
        ...
        if hasattr(self.spot_5m, "set_strategy_logger"):
            self.spot_5m.set_strategy_logger(self.logger, strategy_type='bearish')
        if hasattr(self.futures_5m, "set_strategy_logger"):
            self.futures_5m.set_strategy_logger(self.logger, strategy_type='bearish')

        # Order manager is injected from engine when used.
        self.order_manager = order_manager

        # Primary entry managers
        self.green_candle_manager = GreenCandleEntryManager(self.logger, data_source=self.primary_symbol)
        self.entry_manager = PullBackEntryManager(self.logger, data_source=self.primary_symbol)
        self.mother_child_manager = MotherChildEntryManager(self.logger, data_source=self.primary_symbol)
        self.three_top_manager = ThreeTopEntryManager(self.logger, data_source=self.primary_symbol)

        # Secondary entry managers (run independently on secondary 1m candles)
        self.fut_green_candle_manager = GreenCandleEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_entry_manager = PullBackEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_mother_child_manager = MotherChildEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_three_top_manager = ThreeTopEntryManager(self.logger, data_source=self.secondary_symbol)
    ...
    def generate_signals(self):
        ...
        self.logger.info(
            "Monitoring divergence in real-time | primary=%s secondary=%s",
            self.primary_symbol,
        ...
        self.logger.info(f"[DATA STATUS] {self.primary_symbol} 5m candles: {spot_candles}, Pivot highs: {spot_pivot_highs}")

        # Log ALL pivot highs if any
        if hasattr(self.spot_5m, 'pivot_highs') and self.spot_5m.pivot_highs:
            self.logger.info(f"[PIVOT HIGHS] Found {len(self.spot_5m.pivot_highs)} pivot highs in {self.primary_symbol} data")
    ...
    def check_live_tick(self, price, tick_time=None):
        ...
        if entry_signals:
            for signal in entry_signals:
                ...
                return self._convert_to_trade_signal(signal, price)

        return None

    def check_futures_tick(self, price, tick_time=None):
        """
        Check secondary entry managers for signals using secondary price.

        Args:
            price: Current secondary symbol price
            tick_time: Current timestamp
        ...
        if entry_signals:
            for signal in entry_signals:
                ...
                self.logger.info(
                    f"[{self.secondary_symbol}] Stopped tracking pivot {pivot_number}{candle_str} "
                    f"after {entry_method} entry"
                )
                ...
                if hasattr(self.futures_5m, 'remove_pivot_high_from_tracking'):
                    self.futures_5m.remove_pivot_high_from_tracking(pivot_number)
                self.logger.info(
                    f"[PIVOT TRACKING] Removed pivot high {pivot_number}{candle_str} after {self.secondary_symbol} entry"
                )
    ...
    def check_secondary_tick(self, price, tick_time=None):
        """Alias for engines that route secondary symbol ticks explicitly."""
        return self.check_futures_tick(price, tick_time=tick_time)

    def _is_primary_source(self, data_source):
        return str(data_source).lower() in self._primary_source_aliases

    def _is_secondary_source(self, data_source):
        return str(data_source).lower() in self._secondary_source_aliases
    ...
    def on_candle_close(self, candle, timeframe='1m', data_source='spot'):
        ...
        Args:
            ...
            data_source: spot/primary or futures/secondary aliases
        """
        if timeframe == '1m':
            current_price = candle.get('close', 0)
            if self._is_primary_source(data_source):
            ...
            elif self._is_secondary_source(data_source):
    ...
    def on_candle_correction(self, corrected_candle, data_source='spot'):
        ...
        if self._is_primary_source(data_source):
            ...
            self.logger.warning("Unknown data_source '%s' for candle correction", data_source)
            return
        ...
        try:
            ...
            for spot_pivot in self.spot_5m.pivot_highs:
                ...
                if not existing_fut_pivot:
                    # Get secondary feed high at primary pivot time.
                    fut_price = self._get_secondary_high_at_time(spot_pivot['time'])
                    if fut_price is not None:
                        ...
                        self.logger.info(
                            f"[DERIVED] {self.secondary_symbol} Pivot High {fut_pivot['pivot_number']} (Candle: {fut_pivot['time']}) "
                            f"from {self.primary_symbol} pivot - Price: {fut_pivot['price']:.2f}"
                        )
                        newly_added = True
                ...
                if self.futures_5m.completed_candles:
                    ...
                    for fut_pivot in self.futures_5m.pivot_highs:
                        ...
                        if not already_broken and last_price >= fut_pivot['price']:
                            ...
                            self.logger.info(
                                f"[HISTORICAL BREAKOUT] {self.secondary_symbol} Pivot High {pivot_number} "
    ...
    def _get_secondary_high_at_time(self, pivot_time):
        """
        Resolve the secondary-chart high for a given primary pivot timestamp.
        Supports old futures naming and generic secondary naming.
        """
        for method_name in ("get_secondary_high_at_time", "get_futures_high_at_time"):
            getter = getattr(self.spot_5m, method_name, None)
            if callable(getter):
                return getter(pivot_time)
        self.logger.warning(
            "No secondary high lookup method found on primary feed "
            "(expected get_secondary_high_at_time or get_futures_high_at_time)."
        )
        return None
    ...
    def _check_for_divergence(self, current_price, current_time):
        ...
        for idx, div in enumerate(self.active_divergences[:]):  # Use copy to allow removal during iteration
            ...
            if spot_time and futures_time:
                ...
                self.logger.info(
                    f"📝 Reason: Both {self.primary_symbol} and {self.secondary_symbol} pivot highs are now broken"
                )
                ...
                self.logger.info(
                    f"📝 Reason: Both {self.primary_symbol} and {self.secondary_symbol} pivots are broken"
                )
                self.logger.info("-"*50)
    ...
    def _convert_to_trade_signal(self, signal, current_price):
        """
        Convert divergence signal to plain trade signal.
        """
