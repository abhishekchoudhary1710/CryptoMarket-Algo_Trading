...
class BullishDivergenceStrategy:
    ...
    def __init__(
        ...
        self.entry_manager = RedCandleEntryManager(self.logger, data_source=self.primary_symbol)
        self.pullback_manager = PullBackEntryManager(self.logger, data_source=self.primary_symbol)
        self.mother_child_manager = MotherChildEntryManager(self.logger, data_source=self.primary_symbol)
        self.three_bottom_manager = ThreeBottomEntryManager(self.logger, data_source=self.primary_symbol)

        # Secondary entry managers (run independently on secondary 1m candles)
        self.fut_entry_manager = RedCandleEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_pullback_manager = PullBackEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_mother_child_manager = MotherChildEntryManager(self.logger, data_source=self.secondary_symbol)
        self.fut_three_bottom_manager = ThreeBottomEntryManager(self.logger, data_source=self.secondary_symbol)
    ...
    def generate_signals(self):
        ...
        self.logger.info(f"[DATA STATUS] {self.primary_symbol} 5m candles: {spot_candles}, Pivot lows: {spot_pivot_lows}")

        # Log ALL pivot lows if any
        if hasattr(self.spot_5m, 'pivot_lows') and self.spot_5m.pivot_lows:
            self.logger.info(f"[PIVOT LOWS] Found {len(self.spot_5m.pivot_lows)} pivot lows in {self.primary_symbol} data")
        ...
        if entry_signals:
            for signal in entry_signals:
                ...
                if hasattr(self.futures_5m, 'remove_pivot_from_tracking'):
                    self.futures_5m.remove_pivot_from_tracking(pivot_number)
                self.logger.info(
                    f"[PIVOT TRACKING] Removed pivot {pivot_number}{candle_str} after {self.secondary_symbol} entry"
                )
        ...
        try:
            ...
            for spot_pivot in self.spot_5m.pivot_lows:
                ...
                if not existing_fut_pivot:
                    # Get secondary feed low at primary pivot time.
                    fut_price = self._get_secondary_low_at_time(spot_pivot['time'])
                    if fut_price is not None:
                        ...
                        self.logger.info(
                            f"[DERIVED] {self.secondary_symbol} Pivot Low {pivot_number} (Candle: {fut_pivot['time']}) "
                ...
                if self.futures_5m.completed_candles:
                    ...
                    for fut_pivot in self.futures_5m.pivot_lows:
                        ...
                        if not already_broken and last_price <= fut_pivot['price']:
                            ...
                            self.logger.info(
                                f"[HISTORICAL BREAKDOWN] {self.secondary_symbol} Pivot Low {pivot_number} "
    ...
    def _get_secondary_low_at_time(self, pivot_time):
        """
        Resolve the secondary-chart low for a given primary pivot timestamp.
        Supports old futures naming and generic secondary naming.
        """
        for method_name in ("get_secondary_low_at_time", "get_futures_low_at_time"):
            getter = getattr(self.spot_5m, method_name, None)
            if callable(getter):
                return getter(pivot_time)
        self.logger.warning(
            "No secondary low lookup method found on primary feed "
            "(expected get_secondary_low_at_time or get_futures_low_at_time)."
        )
        return None
    ...
    def _check_for_divergence(self, current_price, current_time):
        ...
        for idx, div in enumerate(self.active_divergences[:]):  # Use copy to allow removal during iteration
            ...
            if spot_time and futures_time:
                ...
                self.logger.info(
                    f"📝 Reason: Both {self.primary_symbol} and {self.secondary_symbol} pivot lows are now broken"
                )
